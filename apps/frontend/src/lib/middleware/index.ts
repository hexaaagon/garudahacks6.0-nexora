...
/**
 * Main middleware handler that manages authentication and authorization
 *
 * Logic:
 * 1. Skip middleware for excluded paths (API routes, static files)
 * 2. If user is authenticated and tries to access auth pages → redirect to portal
 * 3. If user is authenticated and on home page → redirect to portal
 * 4. If route requires auth and user not authenticated → redirect to sign-in
 * 5. If user is authenticated but has no role and not on setup page → redirect to setup
 * 6. If user has role and tries to access setup page → redirect to portal
 * 7. If user tries to access wrong portal → redirect to unauthorized
